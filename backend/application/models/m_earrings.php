...
class m_earrings extends CI_Model 
{ 
	...
	function load_list(){
	$sql = "SELECT DISTINCT * FROM tmtk_earrings a INNER JOIN tmtk_product_uploads b WHERE b.IdUpload=a.UploadFk AND a.sStatusDelete IS NULL AND b.sStatusDelete IS NULL GROUP BY 1";
	...
	function delete(){
		...
		$sql = "call sp_tmtk_earrings('D','".$id."','null','null','null');";
		$this->db->query($sql);
		return null;
	}
	...
	{ 
		...
		$earrings_id = $this->input->post('EarringsId', TRUE); 
		$earrings_name = $this->input->post('EarringsName', TRUE); 
		$earrings_description = $this->input->post('EarringsDescription', TRUE);
		...
		if($hideMode=="U"){
			$sql = "SELECT UploadFk FROM tmtk_earrings WHERE IdProduct = ".$earrings_id."";
			$result= $this->db->query($sql);
		...
		$idmax= "SELECT CASE WHEN COUNT(1) = 0 THEN 1 ELSE MAX(IdProduct) + 1 END as max FROM tmtk_earrings";
	...
	function gf_transact_uploads($data,$upload_id){
		$sReturn = null;
		// $sql  = "call sp_tmtk_product_uploads('D','".$id_fk['0']['UploadFk']."','null','"."uploads/earrings/".$data['upload_data']['file_name']."');";
		$sql  = "call sp_tmtk_product_uploads('I','".$upload_id."','null','"."uploads/earrings/".$data['upload_data']['file_name']."');";
	...
	function get_tag_list($load){
		...
		if($load){
			$load_tag= $this->db->query("SELECT IdAttribute_fk FROM `tmtk_tag` where IdProduct_fk = ".$id_data." and ProductCategory=2 and sStatusDelete is null");
