...
{ 
	...
	if($result->num_rows()>0){
		...
		{
			$query_tag = $this->db->query("SELECT DISTINCT AttributeName FROM tmtk_attribute a INNER JOIN tmtk_tag b ON IdAttribute_fk = IdAttribute WHERE b.IdProduct_fk = '".$row['IdProduct']."' AND b.ProductCategory = 1 AND a.sStatusDelete IS NULL AND b.sStatusDelete IS NULL");
